...
const FriendCard: React.FC<FriendCardProps> = ({ friend, onContact, onDelete, onEdit, onRequestMeeting, feedback, onDismissFeedback }) => {
  ...
  return (
    <div className={`${theme.cardBg} rounded-2xl p-4 shadow-sm border ${theme.border} relative overflow-hidden transition-all duration-200 hover:shadow-md mb-4 group`} role="article" aria-label={`Friend card for ${friend.name}`}>
      <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-2">
        <div className="flex items-center gap-1">
          ...
          <span className={`text-[10px] font-bold uppercase tracking-wider ${theme.surfaceHover} ${theme.textSub} px-2 py-1 rounded-md`}>
            {friend.category}
          </span>
        </div>
      ...
      <div className="flex justify-between items-start mb-3 mt-1 mr-24">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className={`font-bold ${theme.textMain} text-lg leading-tight truncate max-w-[150px]`}>{friend.name}</h3>
      ...
      <div className="mt-3 relative">
        <div className={`flex justify-between text-[10px] font-bold ${theme.textSub} mb-1 px-1`}>
          <span>Needs Water</span>
          <span>Thriving</span>
        </div>
        <div className={`w-full h-3 ${theme.surfaceHover} rounded-full overflow-hidden border ${theme.border}`} role="progressbar" aria-valuenow={Math.round(visualPercentage)} aria-valuemin={0} aria-valuemax={100} aria-label={`Contact timer: ${isOverdue ? `${Math.abs(daysLeft)} days overdue` : `${daysLeft} days remaining`}`}>
      ...
      <button
        ...
        className={`w-full mt-3 p-2 rounded-xl ${theme.surfaceHover} hover:${theme.surfaceActive.replace('bg-', '')} text-xs ${theme.textSub} flex items-center justify-between`}
      >
      ...
      <div className="flex items-center gap-1.5 mt-4 mb-2">
        <span className={`text-[10px] font-bold ${theme.textSub} uppercase tracking-wider mr-1`}>Via:</span>
      ...
      <div className="flex gap-2">
        <button onClick={() => onContact(friend.id, 'REGULAR', selectedChannel)} className={`flex-1 ${theme.primary} ${theme.primaryText} py-2.5 rounded-xl font-medium text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-lg shadow-${theme.primary.replace('bg-', '')}/10 hover:${theme.primaryHover.replace('bg-', '')}`} aria-label={`Water ${friend.name} — log regular contact`}>
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xl transition-colors border ${isDeepCooldown ? `${theme.surfaceHover} ${theme.textDisabled} ${theme.border} cursor-not-allowed` : `text-pink-400 hover:bg-pink-50 hover:text-pink-600 ${theme.border} hover:border-pink-200`}`}
        ...
        <button
          ...
          className={`px-3 py-2 rounded-xl transition-colors border ${!canQuickTouch ? `${theme.surfaceHover} ${theme.textDisabled} ${theme.border} cursor-not-allowed` : `text-yellow-500 hover:bg-yellow-50 hover:text-yellow-600 ${theme.border} hover:border-yellow-200`}`}
        ...
        <button onClick={() => onRequestMeeting(friend)} className={`px-3 py-2 rounded-xl ${theme.textSub} hover:bg-orange-50 hover:text-orange-600 transition-colors border ${theme.border} hover:border-orange-100`} title="Create meeting request" aria-label={`Schedule meeting with ${friend.name}`}>
          <CalendarPlus size={20} />
        </button>

        <button onClick={() => onEdit(friend)} className={`px-3 py-2 rounded-xl ${theme.textSub} hover:bg-blue-50 hover:text-blue-600 transition-colors border ${theme.border} hover:border-blue-100`} title="Edit" aria-label={`Edit ${friend.name}`}>
          <Edit2 size={20} />
        </button>
        ...
        {confirmDelete ? (
          <div className="flex gap-1">
            ...
            <button onClick={() => setConfirmDelete(false)} className={`px-2 py-2 rounded-xl ${theme.surfaceActive} ${theme.textSub} text-[10px] font-bold transition-colors`} title="Cancel">No</button>
          </div>
        ) : (
          <button onClick={() => setConfirmDelete(true)} className={`px-3 py-2 rounded-xl ${theme.textSub} hover:bg-red-50 hover:text-red-500 transition-colors border ${theme.border} hover:border-red-100`} title="Delete">
            <Trash2 size={20} />
          </button>
        )}
      </div>
